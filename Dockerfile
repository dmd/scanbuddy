...
WORKDIR /tmp
RUN dnf install -y --allowerasing libpng15 motif
RUN dnf install -y git make clang zlib-devel libXt-devel libXext-devel expat-devel motif-devel f2c
...
# install utilities for working inside the image if needed
RUN dnf install -y vim tcsh 
